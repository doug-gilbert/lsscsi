...
enum sgj_separator_t {
    ...
    SGJ_SEP_COLON_1_SPACE_RM_PARENS,
};

typedef void * sgj_opaque_p;

/* Apart from the state information at the end of this structure, the earlier
 * fields are initialized from the command line argument given to the --json=
 * option. If there is no such argument then they are initialized as shown. */
typedef struct sgj_state_t {
    ...
    bool pr_string;     /* 's' (def: true) permit sub-object with 'meaning' */
...
/* This function tries to convert the in_name_s C string to the "snake_case"
 * convention so the output jsan_name_s only contains lower case ASCII letters,
 ...
 * Returns json_name_s (i.e. the pointer to the output buffer).
 * Note: strlen(in_name_s) should be <= max_json_name_len . */
char * sgj_convert2snake_rm_parens(const char * in_name_s, char * json_name_s,
                                   int max_json_name_len);
/* Returns name_so_s which only contains lower case ASCII letters, numerals
 ...
 * "output_power_mw". */
char * sgj_convert2snake(const char * in_name_s, char * json_name_s,
                         int max_json_name_len);
/* Is name_s made up of only lower case alphanumerics numbers and
 * underscores? */
bool sgj_is_snake_name(const char * name_s);
 ...
 *    js    - JSON only output
 *    haj   - human readable and JSON output. If pr_out_hr is true then human
 *            readable output is placed in the 'plain_text_output' array
 *    pr    - has printf() style variadic arguments
 *    _r    - suffix indicating the return value should/must be used
 *    nv    - adds a name-value JSON field (or several)
 *    val_o - value is a JSON object (or array)
 *    val_i - value is a JSON integer object (int64_t or uint64_t)
 *    val_b - value is a JSON boolean object
 *    s     - value is a JSON string object
 *    name_so_s - string (pointer) that will become the name of the (sub)
 *                object created. Assumed to be a conforming "snake"
 *                variant JSON name (only lower case and underscore)
 *    name  - string (pointer) that will become the name of the (sub)
 *            object created. If not in a conforming "snake" variant
 *            JSON format, then it is converted into one before be applied.
 *    str   - same as s
 *    hex   - value is a sequence of bytes in hexadecimal within a JSON
 *            string object
...
/* following are similar, useful for leaves (of the tree structure) */
sgj_opaque_p sgj_new_unattached_string_r(sgj_state * jsp, const char * val_s);
sgj_opaque_p sgj_new_unattached_str_len_r(sgj_state * jsp, const char * val_s,
                                          int vlen);
sgj_opaque_p sgj_new_unattached_integer_r(sgj_state * jsp, uint64_t val_i);
sgj_opaque_p sgj_new_unattached_bool_r(sgj_state * jsp, bool val_b);
 ...
 * 'name_so_s' points to) at 'jop' with an empty object as its value; a
 * pointer to that empty object is returned. If 'jop' is NULL then
 * jsp->basep is used instead. The returned value should always be checked
 * as NULL indicates an error has occurred. */
sgj_opaque_p sgj_named_subobject_r(sgj_state * jsp, sgj_opaque_p jop,
                                   const char * name_so_s);
sgj_opaque_p sgj_snake_named_subobject_r(sgj_state * jsp, sgj_opaque_p jop,
                                         const char * conv2sname);

/* If jsp is NULL or jsp->pr_as_json is false nothing happens and NULL is
 * returned. Otherwise it creates a new named object (whose name is what
 * 'name_so_s' points to) at 'jop' with an empty array as its value; a pointer
 * to that empty array is returned.  If 'jop' is NULL then jsp->basep is
 * used instead. The returned value should always be checked as NULL
...
/* If either jsp or val_s is NULL or jsp->pr_as_json is false then nothing
 * happens and NULL is returned. The insertion point is at jop but if it is
 * NULL jsp->basep is used. If 'name_so_s' is non-NULL a new named JSON object
 * is added using 'name_so_s' and the associated value is a JSON string formed
 * from 'val_s'. If 'name_so_s' is NULL then 'jop' is assumed to be a JSON
 * array and a JSON string formed from 'val_s' is added. Note that the
...
sgj_opaque_p sgj_js_nv_s(sgj_state * jsp, sgj_opaque_p jop,
                         const char * name_so_s, const char * val_s);
sgj_opaque_p sgj_js_nv_s_len(sgj_state * jsp, sgj_opaque_p jop,
                             const char * name_so_s,
                             const char * val_s, int val_len);

/* This variant checks 'va_p' for characters that are not permitted in JSON
 ...
 * jsp->basep is used. If 'name_so_s' is non-NULL a new named JSON object is
 * added using 'name_so_s' and the associated value is a JSON integer formed
 * from 'val_i'. If 'name_so_s' is NULL then 'jop' is assumed to be a JSON
 * array and a JSON integer formed from 'val_i' is added. If successful
 * returns a pointer newly formed JSON integer. */
sgj_opaque_p sgj_js_nv_i(sgj_state * jsp, sgj_opaque_p jop,
                         const char * name_so_s, int64_t val_i);

/* If either jsp is NULL or jsp->pr_as_json is false then nothing happens and
 * NULL is returned. The insertion point is at jop but if it is NULL
 * jsp->basep is used. If 'name_so_s' is non-NULL a new named JSON object is
 * added using 'name_so_s' and the associated value is a JSON boolean formed
 * from 'val_b'. If 'name_so_s' is NULL then 'jop' is assumed to be a JSON
 * array and a JSON boolean formed from 'val_b' is added. If successful returns
 * a pointer newly formed JSON boolean. */
sgj_opaque_p sgj_js_nv_b(sgj_state * jsp, sgj_opaque_p jop,
                         const char * name_so_s, bool val_b);

/* If jsp is NULL, jsp->pr_as_json is false or ua_jop is NULL, then nothing
 * happens and NULL is returned. 'jop' is the insertion point but if it is
 * NULL jsp->basep is used instead. If 'name_so_s' is non-NULL a new named JSON
 * object is added using 'name_so_s' and the associated value is ua_jop. If
 * 'name_so_s' is NULL then 'jop' is assumed to be a JSON array and ua_jop is
 * added to it. If successful returns ua_jop . The "ua_" prefix stands for
 * unattached. That should be the case prior to invocation and it will be
 ...
 * if jop is NULL) along with the integer 'val_i'. If jsp->pr_hex is true
 * then that value is two sub-objects, one named 'i' with a 'val_i' as a
 * JSON integer, the other one named 'hex' with 'val_i' rendered as hex in
 * a JSON string. If jsp->pr_hex is false then there are no sub-objects and
 * the 'val_i' is rendered as JSON integer. */
void sgj_js_nv_ihex(sgj_state * jsp, sgj_opaque_p jop,
                    const char * name_so_s, uint64_t val_i);

/* This function only produces JSON output if jsp is non-NULL and
 * jsp->pr_as_json is true. It adds a named object at 'jop' (or jop->basep
 * if jop is NULL) along with 'val_i'. If jsp->pr_string is true then that
 * value is two sub-objects, one named 'i' with a 'val_i' as a JSON integer,
 * the other one named 'nmeaning_or_s' with 'vmeaning_or_s' rendered as a
 * JSON string. If 'nmeaning_or_s' is NULL then "meaning" will be used. If
 * jsp->pr_string is false then there are no sub-objects and 'val_i' is
 * rendered as a JSON integer. */
void sgj_js_nv_istr(sgj_state * jsp, sgj_opaque_p jop,
                    const char * name_so_s, int64_t val_i,
                    const char * nmeaning_or_s, const char * vmeaning_or_s);

/* Similar to sgj_js_nv_istr(). If jsp->pr_hex is true then 'val_i' is
 * rendered as ASCII hex in a JSON string. . */
void sgj_js_nv_ihexstr(sgj_state * jsp, sgj_opaque_p jop,
                       const char * name_so_s, int64_t val_i,
                       const char * nmeaning_or_s, const char * vmeaning_or_s);

/* This function only produces JSON output if jsp && jsp->pr_as_json are true.
 * It adds a named object at 'jop' (or jsp->basep if jop is NULL) along with
 * 'val_i'. If jsp->pr_name_ex is true then 'name_so_s' has two sub-objects,
 ...
 * rendered as a hex string equivalent to 'val_i'. If jsp->pr_name_ex is
 * false and either jsp->pr_hex or 'hex_as_well' are false then there are no
 * sub-objects and the 'val_i' is rendered as a JSON  integer. */
void sgj_js_nv_ihex_nex(sgj_state * jsp, sgj_opaque_p jop,
                        const char * name_so_s, int64_t val_i,
                        bool hex_as_well, const char * nex_s);

void sgj_js_nv_ihexstr_nex(sgj_state * jsp, sgj_opaque_p jop,
                           const char * name_so_s, int64_t val_i,
                           bool hex_as_well, const char * nmeaning_or_s,
                           const char * vmeaning_or_s, const char * nex_s);

/* String value version of above *_nex() functions. If both 'val_s' and
 * 'nex_s' are non-NULL forms sub-object named 'name_so_s' with the names
...
void sgj_js_nv_s_nex(sgj_state * jsp, sgj_opaque_p jop, const char * name_so_s,
                     const char * val_s, const char * nex_s);

/* Add named field whose value is a (large) JSON string made up of num_bytes
 * ASCII hexadecimal bytes (each two hex digits and separated by a space)
 * starting at byte_arr. The heap is used for intermediate storage so
 * num_bytes can be arbitrarily large. */
void sgj_js_nv_hex_bytes(sgj_state * jsp, sgj_opaque_p jop,
                         const char * name_so_s, const uint8_t * byte_arr,
                         int num_bytes);
 ...
 * leadin_sp spaces followed by 'name' then a separator, then 'val_s' with a
 ...
 * then jsp->basep is used. If 'name' is NULL then a JSON string object,
 * made from 'val_s' is added to the JSON array pointed to by 'jop'.
 * Otherwise a 'name' JSON object whose value is a JSON string object made
 * from 'val_s' is added at 'jop'. */
...
void sgj_haj_vi(sgj_state * jsp, sgj_opaque_p jop, int leadin_sp,
                const char * name, enum sgj_separator_t sep,
                int64_t val_i, bool hex_haj);
void sgj_haj_vistr(sgj_state * jsp, sgj_opaque_p jop, int leadin_sp,
                   const char * name, enum sgj_separator_t sep,
                   int64_t val_i, bool hex_haj, const char * vmeaning_s);
...
void sgj_haj_vi_nex(sgj_state * jsp, sgj_opaque_p jop, int leadin_sp,
                    const char * name, enum sgj_separator_t sep,
                    int64_t val_i, bool hex_haj, const char * nex_s);
void sgj_haj_vistr_nex(sgj_state * jsp, sgj_opaque_p jop, int leadin_sp,
                       const char * name, enum sgj_separator_t sep,
                       int64_t val_i, bool hex_haj,
                       const char * vmeaning_s, const char * nex_s);

/* Similar to above '_haj_' calls but a named sub-object is always formed
 * containing a JSON integer object named "i" whose value is 'val_i'. The
 ...
 * from 'sp' until either a '\0' is found or 'slen' is exhausted. Adds each
 * line to jsp->out_hrp JSON array (if conditions met). Outputs to stdout. */
 ...
 * is true then a new JSON object named "exit_status" is added and its value
 * is 'exit_status' value rendered as a JSON integer. The 'exit_status' JSON
 * object is appended to 'jop' or jsp_basep if 'jop' is NULL. The in-core
 ...
 * if char_if_not_j is zero, that '-j' is the short form of the --json[=JO]
 * command line option.  */
